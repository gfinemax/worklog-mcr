...
async function fixSessionRoles() {
    ...
    if (!members || members.length === 0) {
        console.log('Session has no members.')
        return
    }
    ...
    const parkUser = users?.find(u => u.name === '박상필')
    const kimUser = users?.find(u => u.name === '김준일')

    const parkMember = members.find(m => m.user_id === parkUser?.id)
    const kimMember = members.find(m => m.user_id === kimUser?.id)

    if (parkMember && kimMember) {
        ...
        console.log('Could not find Park or Kim member records.')
    }
}
