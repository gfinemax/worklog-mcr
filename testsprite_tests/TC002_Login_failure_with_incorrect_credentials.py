...
async def run_test():
    ...
    try:
        ...
        # -> Click the '게스트/관리자 로그인' (Guest/Admin Login) button to go to the login page.
        frame = context.pages[-1]
        # Click the '게스트/관리자 로그인' (Guest/Admin Login) button to navigate to the login page
        elem = frame.locator('xpath=html/body/div[2]/aside/div/div[2]/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input invalid email and password, then click the login button to test login failure.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Click the 로그인 (login) button to attempt login with invalid credentials
        ...
        try:
            ...
            raise AssertionError('Test case failed: Login should fail with incorrect username or password, but the success message was not found, indicating the login was incorrectly allowed.')
